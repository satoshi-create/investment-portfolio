...
export function InventoryTable({
  ...
  return (
    <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-2xl">
        ...
        <div className="divide-y divide-border/60">
          {sortedStocks.length === 0 ? (
            ...
            sortedStocks.map((stock) => {
              const mv =
                stock.marketValue > 0
                  ? formatJpyValueForView(stock.marketValue, viewCurrency, convert)
                  : "—";
              const pnlAbs = Math.abs(stock.unrealizedPnlJpy ?? 0);
              const pnlText =
                stock.unrealizedPnlJpy != null && Number.isFinite(stock.unrealizedPnlJpy)
                  ? `${stock.unrealizedPnlJpy > 0 ? "+" : stock.unrealizedPnlJpy < 0 ? "−" : ""}${formatJpyValueForView(
                      pnlAbs,
                      viewCurrency,
                      convert,
                    )}`
                ...
                <div key={stock.id} className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-[11px] text-muted-foreground truncate">
                        {stock.tag?.trim()?.length ? stock.tag : "—"}
                      </div>
                    </div>
                    {onTrade ? (
                      <button
                        ...
                        className="shrink-0 text-[10px] font-bold uppercase tracking-wide text-accent-cyan border border-accent-cyan/40 px-3 py-2 rounded-lg hover:bg-accent-cyan/10"
                      >
                        Trade
                      </button>
                    ) : null}
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-2">
                    ...
                    <div className="rounded-xl border border-border bg-background/40 px-3 py-2">
                      ...
                      <div
                        className={`mt-1 font-mono text-xs font-semibold ${signedValueClass(
                          stock.unrealizedPnlJpy ?? 0,
                        )}`}
                      >
                        {pnlText}
                      </div>
                    ...
                    <div className="rounded-xl border border-border bg-background/40 px-3 py-2">
                      ...
                      <div
                        className={`mt-1 font-mono text-xs font-semibold ${signedPctClass(
                          stock.dayChangePercent,
                        )}`}
                      >
                        {formatSignedPercent(stock.dayChangePercent)}
                      </div>
                    </div>
                  </div>
      ...
      <div className="hidden md:block relative w-full max-w-full overflow-x-auto overscroll-x-contain touch-auto [-webkit-overflow-scrolling:touch]">
