...
type HoldingQueryRow = {
  ...
  /** DB `holdings.institutional_ownership` */
...
async function fetchHoldingsRowsWithInvestmentMeta(db: Client, userId: string) {
  ...
  async function runWithInstitutionalFallback(frag: string) {
    try {
  ...
  try {
    return await runWithInstitutionalFallback(`${core}${meta}${shortTerm}`);
  } catch (e) {
    if (holdingsMissingShortTermRulesColumns(e)) {
      try {
        return await runWithInstitutionalFallback(`${core}${meta}`);
      } catch (e2) {
      ...
      try {
        rs = await db.execute({
          sql: `SELECT ticker, revenue_growth, fcf_margin, fcf_yield, fcf
              , listing_date, market_cap, listing_price, next_earnings_date, memo, is_bookmarked, instrument_meta_synced_at, earnings_summary_note, chasm, moat, vi_score, institutional_ownership, lynch_drivers_narrative, lynch_story_text FROM theme_ecosystem_members
              WHERE ticker IN (${ph})`,
          args: unique,
        });
      } catch (eEco) {
...
function buildDraftsFromHoldingRows(
  ...
  return rows.map((row) => {
    ...
    const liveInst = research?.institutionalOwnership;
    const dbInstitutional = parseOptionalFiniteNumberMeta(row["institutional_ownership"]);
    const institutionalOwnership =
      liveInst != null && Number.isFinite(liveInst) ? liveInst : dbInstitutional;
...
function parseEcosystemChasmMoatText(raw: unknown): string | null {
  if (raw == null) return null;
  const s = String(raw).trim();
  return s.length > 0 ? s : null;
}

function parseEcosystemViScore0to100(raw: unknown): number | null {
  if (raw == null) return null;
  const n = Number(raw);
...
async function enrichEcosystemMemberRow(
  ...
  const observationNotes =
    row["observation_notes"] != null && String(row["observation_notes"]).trim().length > 0
      ...
      : null;
  const chasmEco = parseEcosystemChasmMoatText(row["chasm"]);
  const moatEco = parseEcosystemChasmMoatText(row["moat"]);
  const viScoreEco = parseEcosystemViScore0to100(row["vi_score"] ?? row["viScore"]);
  ...
  const earningsSummaryNoteEco =
    rawEarnNote != null && String(rawEarnNote).trim().length > 0
      ...
      : null;
  const rawLynchDrv = row["lynch_drivers_narrative"];
  const lynchDriversNarrativeEco =
    rawLynchDrv != null && String(rawLynchDrv).trim().length > 0 ? String(rawLynchDrv) : null;
  ...
  const liveInstEco = research?.institutionalOwnership;
  const dbInstEco = parseOptionalFiniteNumberMeta(row["institutional_ownership"]);
  const institutionalOwnershipEco =
    liveInstEco != null && Number.isFinite(liveInstEco) ? liveInstEco : dbInstEco;
  const daysToEarnings = computeUtcCalendarDaysUntil(nextEarningsDate);
  ...
  return {
    ...
    institutionalOwnership: institutionalOwnershipEco,
    observationStartedAt,
    alphaHistory,
    ...
    try {
      const rs = await db.execute({
        sql: `SELECT m.id, m.theme_id, m.ticker, m.is_unlisted, m.proxy_ticker, m.estimated_ipo_date, m.estimated_valuation,
              ...
              , m.listing_date, m.market_cap, m.listing_price, m.next_earnings_date, m.memo, m.is_bookmarked, m.instrument_meta_synced_at, m.earnings_summary_note, m.chasm, m.moat, m.vi_score, m.lynch_drivers_narrative, m.lynch_story_text
