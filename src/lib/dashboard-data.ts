      ...
      try {
        rs = await db.execute({
          sql: `SELECT ticker, revenue_growth, fcf_margin, fcf_yield, fcf
              , listing_date, market_cap, listing_price, next_earnings_date, memo, is_bookmarked, instrument_meta_synced_at, earnings_summary_note, chasm, moat, vi_score, institutional_ownership FROM theme_ecosystem_members
              WHERE ticker IN (${ph})`,
          args: unique,
        });
      } catch (eEco) {
    ...
    try {
      const rs = await db.execute({
        sql: `SELECT m.id, m.theme_id, m.ticker, m.is_unlisted, m.proxy_ticker, m.estimated_ipo_date, m.estimated_valuation,
              ...
              , m.listing_date, m.market_cap, m.listing_price, m.next_earnings_date, m.memo, m.is_bookmarked, m.instrument_meta_synced_at, m.earnings_summary_note, m.chasm, m.moat, m.vi_score
